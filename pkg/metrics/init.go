...
func Setup(ctx context.Context, logger *zap.SugaredLogger) error {
	var setupErr error
	initOnce.Do(func() {
		// Initialize OTel MeterProvider with Prometheus exporter and start metrics HTTP server
		if err := setupPrometheusExporterAndServer(logger); err != nil {
			setupErr = err
			return
		}

		// Initialize the metrics reporter (instruments only)
		...
		logger.Info("Observability initialized (OpenTelemetry Prometheus exporter on :9090)")
	})
...
func setupPrometheusExporterAndServer(logger *zap.SugaredLogger) error {
	var initErr error
	serverOnce.Do(func() {
		// Create Prometheus exporter and set global MeterProvider
		var err error
		...
		// Start HTTP server for metrics on :9090 using promhttp default handler
		mux := http.NewServeMux()
		mux.Handle("/metrics", promhttp.Handler())
		go func() {
			//nolint:gosec // in-cluster metrics endpoint is intentionally served without TLS
...
// FlushMetrics flushes any pending metrics (no-op)
func FlushMetrics() {}
