...
// - Knative controller metrics (automatically via sharedmain/injection)
...
// ReportReconcile records reconciliation metrics.
// Knative controller metrics are emitted automatically by knative.dev/pkg; here we record pruner-specific OTEL metrics.
...
// ReportQueueDepth reports current queue depth to OpenTelemetry for consistency.
// Knative workqueue metrics are tracked automatically via the k8s metrics provider.
...
func (hr *HybridReporter) GetMetricsSummary() map[string]interface{} {
	return map[string]interface{}{
		...
		"knative_controller_metrics": map[string]interface{}{
			"note":  "emitted automatically by knative.dev/pkg",
			"count": 12,
		},
